	...
	for {
		...
		case event := <-watcher.Events():
			...
			CheckWhitelist:
				...
				if whitelisted {
					log.Printf("User %q in whitelist, approving request...", event.Request.User)
					params.State = workflows.StateApproved
					params.Reason = "user in whitelist"
				} else {
					log.Printf("User %q not in whitelist, denying request...", event.Request.User)
					params.State = workflows.StateDenied
					params.Reason = "user not in whitelist"
				}
				if err := plugin.SetRequestState(ctx, event.Request.ID, "delegator", params); err != nil {
					return trace.Wrap(err)
				}
			...
			case workflows.OpDelete:
				...
				log.Printf("Request %s has automatically expired.", event.Request.ID)
			default:
